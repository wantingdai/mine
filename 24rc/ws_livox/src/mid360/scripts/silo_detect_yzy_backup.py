...
def PointCloudCallback(data):
    ...
    # msg, x, y, yaw = ros_recv.split_ros_msg(ros_recv.read_recv())
    cmd, x, y, yaw = ros_recv.split_ros_msg(ros_recv.read_recv())
    # print(cmd, x, y, yaw)

    # 修正坐标系
    # real_x = (-y) * math.sin(math.radians(yaw)) - (-x) * math.cos(math.radians(yaw))
    # real_y = (-y) * math.cos(math.radians(yaw)) + (-x) * math.sin(math.radians(yaw))
    ...
    silo_x_min = [0, 0, 0, 0, 0]
    silo_x_max = [0, 0, 0, 0, 0]
    silo_y_min = [0, 0, 0, 0, 0]
    silo_y_max = [0, 0, 0, 0, 0]
    real_x_min = [0, 0, 0, 0, 0]
    real_x_max = [0, 0, 0, 0, 0]
    real_y_min = [0, 0, 0, 0, 0]
    real_y_max = [0, 0, 0, 0, 0]

    for i in range(5):
        silo_x_min[i] = -x + CENTER_POS[i] + X_MIN
        silo_x_max[i] = -x + CENTER_POS[i] + X_MAX
        silo_y_min[i] = -y + Y_MIN
        silo_y_max[i] = -y + Y_MAX
        if yaw < 0: # 同上，也是取的外切部分，如果取外切部分，则x_min和y_max作为real_x_min，取x_max和y_max作为real_y_max
            real_x_min[i] = silo_x_min[i] * math.cos(math.radians(yaw)) + silo_y_max[i] * math.sin(math.radians(yaw))
            real_x_max[i] = silo_x_max[i] * math.cos(math.radians(yaw)) + silo_y_min[i] * math.sin(math.radians(yaw))
            real_y_min[i] = silo_y_min[i] * math.cos(math.radians(yaw)) - silo_x_min[i] * math.sin(math.radians(yaw))
            real_y_max[i] = silo_y_max[i] * math.cos(math.radians(yaw)) - silo_x_max[i] * math.sin(math.radians(yaw))
        elif yaw > 0:  # 此处取法是取的框的外切部分，如果出现漏标注，则尝试换成框的外切部分，即取x_min和y_min作为real_x_min，取x_min和y_max作为real_y_max
            real_x_min[i] = silo_x_min[i] * math.cos(math.radians(yaw)) + silo_y_min[i] * math.sin(math.radians(yaw))
            real_x_max[i] = silo_x_max[i] * math.cos(math.radians(yaw)) + silo_y_max[i] * math.sin(math.radians(yaw))
            real_y_min[i] = silo_y_min[i] * math.cos(math.radians(yaw)) - silo_x_max[i] * math.sin(math.radians(yaw))
            real_y_max[i] = silo_y_max[i] * math.cos(math.radians(yaw)) - silo_x_min[i] * math.sin(math.radians(yaw))
    ...
    if cmd == 1:
        ...
        for p in gen:
            ...
            for silo in range(5):
                if real_y_min[silo] < p[1] < real_y_max[silo] and (HEIGHT_MIN < p[2] < HEIGHT_MAX):  # y,z limit
                    if real_x_min[silo] < p[0] < real_x_max[silo]:  # x limit
                        if 0 < p[-1] < 20:  # intensity limit
                            for i in range(LAYER_NUM):  # 遍历框范围中的点，判断其所在的层
                                if LAYERS[i] <= p[2] < LAYERS[i + 1]:
                                    layer_point_cnt[silo][i] += 1
...
def pc_listener():
    rospy.init_node('silo_detector_body', anonymous=True)
    rospy.Subscriber("/cloud_registered_body", PointCloud2, PointCloudCallback, queue_size=10)
    rospy.spin()
