...
def PointCloudCallback(data):
    ...
    if cmd == 1:
        ...
        for p in gen:
            p = list(p)  # 元组不可以直接修改值，改为列表
            if MODE != 'DEV':  # 修正为顺时针旋转90°的坐标系，修正后的坐标系用u、v、w表示
            ...
            # 修正坐标（局部转世界）
            p[0], p[1] = trans_cond(p[0], p[1], x, y, yaw)
            for silo in range(5):
                    ...
                    if CENTER_POS[silo] + X_MIN < p[0] < CENTER_POS[silo] + X_MAX:  # x limit
                        if p[-1] < 50:  # intensity limit
                            for i in range(LAYER_NUM):  # 遍历框范围中的点，判断其所在的层
                                if LAYERS[i][0] <= p[2] < LAYERS[i][1]:
                                # if LAYERS[i] + 0.1 <= p[2] < LAYERS[i + 1] - 0.02:  # 做略微分离
...
if __name__ == '__main__':
    ...
    HEIGHT_MIN, HEIGHT_MAX = -0.10, 0.45
    DIAMETER = 0.25
    GAP = 0.5
    ...
    LAYERS = [[0.03, 0.11], [0.13, 0.22], [0.33, 0.45]]
